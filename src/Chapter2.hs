...
subList i j list
  | i < 0 || j < 0 = []
  | otherwise = take (j - i + 1) (drop i list)
...
Nearest power of 2 which is greater than the input?
...
duplicate list = go [] list
  where
    go :: [a] -> [a] -> [a]
    go acc [] = acc
    go acc (x:xs) = go (acc ++ replicate 2 x) xs
...
contains needle listOfLists = filter (elem needle) listOfLists
